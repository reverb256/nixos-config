...
void WorkerManager::handle_login(Connection* conn, const StratumRequest& req) {
    ...
    if (has_job_) {
        ...
        std::string response = R"({"id": )" + std::to_string(req.id) +
            R"(, "jsonrpc": "2.0", "result": {"id": ")" + json_escape(worker_id) +
...
void WorkerManager::send_job(const Job& job) {
    ...
    for (auto& [fd, info] : workers_) {
        ...
        if (info.pending_login) {
            ...
            std::string response = R"({"id": 1, "jsonrpc": "2.0", "result": {"id": ")" + json_escape(info.worker_id) +
                R"(", "job": {"algo": "cuckaroo", "blob": ")" + json_escape(worker_blob) +
                R"(", "job_id": ")" + json_escape(job.job_id) +
            ...
            std::string notify = R"({"jsonrpc": "2.0", "method": "job", "params": {"algo": "cuckaroo", "job_id": ")" +
                json_escape(job.job_id) + R"(", "blob": ")" + json_escape(worker_blob) +
                R"(", "target": ")" + json_escape(job.target) +

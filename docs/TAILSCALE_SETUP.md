...
- **Subnet routing** for the 10.1.1.0/24 network
...
services.tailscale-custom = {
  enable = true;
  advertiseRoutes = ["10.1.1.0/24"];      # Advertise local subnet
...
### ✅ Subnet Routing
All nodes advertise the 10.1.1.0/24 network, enabling:
...
```bash
ssh 100.86.158.18          # Connect to nexus
...
tailscale up --exit-node=100.81.182.5
...
Subnet 10.1.1.0/24 is advertised by all nodes.

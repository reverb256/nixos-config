...
| D | D2 | Verified pods start on all 4 nodes — added nixos-cluster-mcp to system packages so closure is built on every node during NixOS rebuild |
...
| D | D2 | Verified pods start on all 4 nodes without manual nix copy | ✅ Done |

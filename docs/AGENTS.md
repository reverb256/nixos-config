...
**Commit:** Latest - Distributed Build System Complete
...
- **RAM:** 64GB
- **Role:** Desktop workstation + mining + VR gaming
...
- **RAM:** 32GB
- **Role:** Distributed builds + backup storage + mining
...
- **RAM:** 32GB
- **Role:** GPU compute cluster node + mining
...
│   ├── forge/                    # GPU compute node (16 cores, dual GPU)
│   ├── nexus/                    # Build/backup server (24 cores, dual GPU)
│   ├── sentry/                   # Monitoring node (8 cores)
│   └── zephyr/                  # Master workstation (32 cores, RTX 3090)
├── modules/                      # Modular configuration (23 files)
...
│   ├── gaming.nix                 # VR/SteamVR/WiVRn + smart mining pause (372 lines)
...
│   ├── networking.nix             # Static IP + VRChat blocklist + Unbound DNS
│   ├── ssh.nix                    # SSH server configuration
│   ├── storage.nix               # Storage and filesystem setup (361 lines)
│   ├── system-packages.nix        # Centralized package list (159 lines)
│   ├── systemd-slices.nix         # Workload isolation slices
│   ├── users.nix                 # User accounts, groups, sudo rules (113 lines)
...
| services.clawdbot | AttrSet | modules/clawdbot-enhanced.nix | AI assistant service |

...
| TPM 2.0 age-key binding | The static cluster age key (SecretSpec/sops identity) is sealed to TPM 2.0 PCRs 0+7 at boot, unsealed into `/run/secrets/cluster-age-key`, and consumed via the existing `SOPS_AGE_KEY_FILE` env var. Does NOT modify SecretSpec — sits below it as a key-provisioning layer. Complements, not replaces, YubiKey age recipients. All 4 hosts have TPM 2.0; all 4 currently have Secure Boot disabled (PCR 7 reflects "SB off" state). | [`../docs/reference/known-issues.md`](../docs/reference/known-issues.md), [`../modules/system/tpm2-age-binding.nix`](../modules/system/tpm2-age-binding.nix), [`../SOPS-NIX.md`](../SOPS-NIX.md#TPM-2.0-hardware-binding) |

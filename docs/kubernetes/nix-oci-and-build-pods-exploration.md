...
**Current State:** The repository uses SSH-based distributed builds declared by
`modules/system/distributed-builds.nix`. That module generates Nexus and Sentry
as remote builders; Forge is excluded from the generated remote-builder list
because it is the GPU/mining host. No Nix build pods are configured in
Kubernetes yet.
...
**File:** `/etc/nix/machines` (generated by the active NixOS module)

The exact entries depend on the host because the module excludes the local
host. The source configuration advertises:

- Nexus: `x86_64-linux`, 12 jobs, speed factor 10, `big-parallel,kvm`
- Sentry: `x86_64-linux`, 8 jobs, speed factor 6, `big-parallel`
- Forge: excluded from the generated remote-builder list

The checked-in root `machines` file is the Nix machines file supplied to
Colmena via `colmena.nix`; it is separate from the generated runtime file.

Nix platform compatibility (`x86_64-linux`) is separate from compiler
optimization. Only selected llama.cpp packages use `-march=x86-64-v3`; the
CachyOS kernel package is explicitly x86-64-v3 optimized.
...
- ✅ SSH-based distributed builds via the NixOS module
- ✅ Nexus and Sentry are the configured remote builders
- ✅ Forge is excluded from the generated builder list to protect GPU mining

# NixOS Cluster Documentation

## Reference

| Document | Purpose |
|----------|---------|
| [AGENTS.md](/AGENTS.md) | AI agent guide (main reference) |
| [README.md](/README.md) | Project overview |

## Operations

| Document | Purpose |
|----------|---------|
| [CLUSTER_STATUS.md](CLUSTER_STATUS.md) | Cluster status |
| [DEPLOYMENT_INSTRUCTIONS.md](DEPLOYMENT_INSTRUCTIONS.md) | Deployment guide |
| [HARSH_REALITY_AUDIT.md](HARSH_REALITY_AUDIT.md) | Infrastructure audit |
| [INCIDENT_RESPONSE_PLAN.md](INCIDENT_RESPONSE_PLAN.md) | Incident response |

## Configuration

| Document | Purpose |
|----------|---------|
| [SETUP.md](SETUP.md) | Initial setup |
| [SECURITY.md](SECURITY.md) | Security configuration |
| [MINING.md](MINING.md) | Mining configuration |
| [PERIPHERALS.md](PERIPHERALS.md) | Razer/Corsair devices |
| [GPU_BUILDS.md](GPU_BUILDS.md) | GPU build configuration |
| [WHISPER_DICTATION.md](WHISPER_DICTATION.md) | Speech-to-text setup |

---
**Total docs:** 12 (down from 116)
**Last updated:** 2026-02-12

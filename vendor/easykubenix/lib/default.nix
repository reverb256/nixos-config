self: lib: {
  ...
  mkNamedList = attrs:
    if !lib.isAttrs attrs
    then throw "mkNamedList error: Input must be an attribute set."
    # Ensure all children are also attribute sets, as required by the `_namedlist` contract.
    else if !(lib.all lib.isAttrs (lib.attrValues attrs))
    then throw "mkNamedList error: All values in the attribute set must themselves be attribute sets."
    else attrs // {_namedlist = true;};
  ...
  mkNumberedList = attrs:
    if !lib.isAttrs attrs
    then throw "mkNumberedList error: Input must be an attribute set."
    else if !(lib.all lib.isAttrs (lib.attrValues attrs))
    then throw "mkNumberedList error: All values in the attribute set must themselves be attribute sets."
    else let
    ...
    in
      if !allKeysAreInts
      then throw "mkNumberedList error: All keys in the attribute set must be integer strings."
      else attrs // {_numberedlist = true;};
  ...
  walkWithPath = transformer: let
    ...
    in
      if lib.isAttrs v'
      then lib.mapAttrs (name: val: go (path ++ [name]) val) v'
      else if lib.isList v'
      then lib.imap0 (index: val: go (path ++ [(toString index)]) val) v'
      else v';
  in
    go [];
  ...
  kubeAttrsToLists = path: value:
    if lib.isAttrs value
    then
      if value._namedlist or false == true
      then
        ...
        lib.mapAttrsToList (
          name: val:
            if !lib.isAttrs val
            then throw "namedListToList error: Value for key '${name}' is not an attribute set."
            else val // {inherit name;}
        ) (lib.removeAttrs value ["_namedlist"])
      else if value._numberedlist or false == true
      then

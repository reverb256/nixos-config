  ...
  config.kubernetes.objects = {
        ...
        template = {
          ...
          spec = {
            ...
            containers.oauth2-proxy = {
              ...
              ports.http = {containerPort = 4180; protocol = "TCP";};
              env._namedlist = true;
              ...
              volumeMounts.secrets = {mountPath = "/etc/oauth2/secrets"; readOnly = true;};
              volumeMounts.ca-cert = {mountPath = "/etc/ssl/cluster-ca"; readOnly = true;};
            };
            volumes._namedlist = true;
            volumes.secrets.secret.secretName = "oauth2-proxy-secrets";
            volumes.ca-cert.hostPath = {path = "/etc/ssl/cluster-ca"; type = "Directory";};
          };
        };
      };
    };
    ...
    auth.Service.oauth2-proxy = {
      ...
      spec = {
        ...
        ports._namedlist = true;
        ports.http = {port = 4180; targetPort = 4180; protocol = "TCP";};
      };
    };
    ...
    auth.NetworkPolicy.oauth2-proxy-ingress = {
      ...
      spec = {
        ...
        policyTypes = ["Ingress"];
        ingress = [
          {from = [{ipBlock.cidr = cluster.podCidr;}]; ports = [{port = 4180; protocol = "TCP";}];}
          {from = [{ipBlock.cidr = cluster.subnet;}]; ports = [{port = 4180; protocol = "TCP";}];}
        ];
      };
    };
    ...
    auth.NetworkPolicy.oauth2-proxy-egress = {
      ...
      spec = {
        ...
        egress = [
          ...
          {to = [{podSelector.matchLabels.app = "casdoor";}]; ports = [{port = 8000; protocol = "TCP";}];}
          {to = [{ipBlock.cidr = cluster.subnet;}]; ports = [{port = 443; protocol = "TCP";}];}
        ];
      };
    };
  };

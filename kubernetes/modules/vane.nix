{ cluster, nexusPreferredAffinity, ... }: let
  labels = {
    app = "vane";
...
in {
  config.kubernetes.objects = {
        ...
        template = {
          ...
          spec = {
            ...
            containers = [{
              ...
              env = [
                { name = "GATEWAY_URL"; value = "http://ai-gateway.ai-inference.svc.cluster.local:8080"; }
                { name = "SEARXNG_URL"; value = "http://searxng.search.svc.cluster.local:8080"; }
                { name = "CROSS_ENCODER_DEVICE"; value = "cpu"; }
    ...
    ai-inference.Deployment.fusion = {
      metadata = { labels = { app = "fusion"; "app.kubernetes.io/managed-by" = "easykubenix"; }; };
      spec = {
        replicas = 1;
        revisionHistoryLimit = 2;
        ...
        template = {
          ...
          spec = {
            ...
            containers = [{
              ...
              env = [
                { name = "KF_API_URL"; value = "http://knowledge-fabric.ai-inference.svc.cluster.local:3100"; }
                { name = "SEARXNG_URL"; value = "http://searxng.search.svc.cluster.local:8080"; }
                { name = "GATEWAY_URL"; value = "http://ai-gateway.ai-inference.svc.cluster.local:8080"; }
    ...
    ai-inference.Service.fusion = {
      ...
      spec = {
        selector = { app = "fusion"; };
        ports = [{ name = "http"; port = 8085; targetPort = 8085; protocol = "TCP"; }];
      };
    };
  };
}

  ...
  nodeDashboard = builtins.toJSON {
    ...
    panels = [
      ...
      }
      {
        ...
        targets = [
          { expr = ''rate(node_network_receive_bytes_total{device=~"eth.*|enp.*"}[5m])''; legendFormat = "{{instance}} RX"; datasource = { type = "prometheus"; uid = "mimir"; }; }
          { expr = ''rate(node_network_transmit_bytes_total{device=~"eth.*|enp.*"}[5m])''; legendFormat = "{{instance}} TX"; datasource = { type = "prometheus"; uid = "mimir"; }; }
        ];
      }
    ];
    schemaVersion = 39;

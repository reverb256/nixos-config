...
- **Static IPs**: `10.1.1.110`, `10.1.1.120`, `10.1.1.130`, `10.1.1.140`
- **Network range**: `10.1.1.0/24`
- **Gateway**: `10.1.1.1`

### 2. Tailscale IPs
- **Node IPs**: `100.81.182.5`, `100.86.158.18`, `100.116.190.124`, `100.82.210.39`

### 3. Hostnames
- **Machine names**: `zephyr`, `nexus`, `forge`, `sentry`

### 4. Mining Wallet IDs
- **Wallet addresses**: `krxXVNVMM7.zephyr`, `krxXVNVMM7.nexus`, `krxXVNVMM7.forge`, `krxXVNVMM7.sentry`
...
```nix
{ config, pkgs, ... }: {
  services.mining.wallet = "krxXVNVMM7.zephyr";  # Your actual wallet
  networking.privateIP = "10.1.1.110";
}
```
...
{ config, lib, ... }: {
  ...
  networking.interfaces.enp38s0.ipv4.addresses = [{
    address = config.myPrivate.nodeIP or "10.0.0.100";
    prefixLength = 24;
  }];
}
```
...
{ 
  myPrivate.hostname = "zephyr";
  myPrivate.nodeIP = "10.1.1.110";
}
```
...
- Internal IP ranges like `10.0.0.0/8` are RFC1918 private addresses but still reveal network structure

...
### 1. Generate MinIO Root Credentials (Optional - Auto-Generated by Default)

**Note**: Credentials are **automatically generated** by the `services.aistor-secrets` module on first boot. No manual setup is required for the default configuration.

If you want to customize credentials, you can do this via:
...
# Option C: Declarative generation (EASIEST - auto-generates on boot)
# The configuration already includes `services.aistor-secrets` which will:
# - Generate secure random credentials on first boot
# - Store them at /tmp/aistor-credentials.env
# - Credentials are generated automatically, no manual steps needed
# To customize, edit services.aistor-secrets.mode: "generate" | "demo" | "custom"
```

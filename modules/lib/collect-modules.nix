...
let
  ...
  excludeFiles = {
    ...
    "development/ai-coding-tools/claude.nix" = true;             # Submodule: imported by ai-coding-tools.nix
    "development/ai-coding-tools/crush.nix" = true;              # Submodule: imported by ai-coding-tools.nix
    ...
    builtins.foldl' (
      acc: name:
        ...
        if entries.${name} == "directory" then
          ...
          if hasAttr name skipDirs then acc
          else acc ++ walk path "${rel}/"
        else if hasSuffix ".nix" name && !hasAttr rel excludeFiles then

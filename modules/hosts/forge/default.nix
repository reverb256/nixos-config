# forge host wiring - TWO-LAYER (host-wiring Q1 to B), Variant B (path-import).
#
...
#           forgeConfig + base + feature imports via withSystem.
{
  ...
  # hosts/forge/configuration.nix plus the classic modules/default.nix
  ...
  flake.nixosConfigurations.forge = withSystem "x86_64-linux" ({system, ...}: let
  ...
  in
    (mkDendriticHost.mkHost {
      hostConfig = config.flake.modules.nixos.forgeConfig;
      extraModules = (import ../../../contracts/host-inventory.nix).hosts.forge.extraModules;
      inherit system;
    }).system);
}

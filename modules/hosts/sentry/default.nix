# Sentry host wiring — TWO-LAYER (host-wiring Q1 → B), Variant B (path-import).
#
...
#           sentryConfig + base + feature imports via withSystem.
{
  ...
  # hosts/sentry/configuration.nix (monitoring.nix, preservation.nix,
  # k3s-cluster.nix, etc.) plus the classic modules/default.nix registry, so
  # no extra nested imports needed until the ~90 shared-file wraps land
  ...
  flake.nixosConfigurations.sentry = withSystem "x86_64-linux" ({system, ...}: let
    ...
    # inventory's per-host extraModules; sentry = none).
    sentryExtraModules = [];
  in
    inputs.nixpkgs.lib.nixosSystem {
      ...
      modules =
        ...
        ++ [
          config.flake.modules.nixos.sentryConfig
        ]
        ++ sentryExtraModules;
    });
}

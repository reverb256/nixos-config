# zephyr host wiring - TWO-LAYER (host-wiring Q1 to B), Variant B (path-import).
#
# Original host wiring (host #1 cutover, #398). Variant B:
# tests/k3s-topology-evidence.nix does builtins.readFile on
...
#           zephyrConfig + base + feature imports via withSystem.
{
  ...
  # hosts/zephyr/configuration.nix plus the classic modules/default.nix
  ...
  flake.nixosConfigurations.zephyr = withSystem "x86_64-linux" ({system, ...}: let
  ...
  in
    (mkDendriticHost.mkHost {
      hostConfig = config.flake.modules.nixos.zephyrConfig;
      extraModules = (import ../../../contracts/host-inventory.nix).hosts.zephyr.extraModules;
      inherit system;
    }).system);
}

...
# Exports xmrig CPU mining metrics for cluster monitoring
# Polls mining APIs and exposes metrics for hashrate, shares, uptime
{
  config,
  lib,
  pkgs,
...
let
  ...
  # Python HTTP server script for serving metrics
...
in
{
  options.services.mining-exporter = {
    ...
    xmrigPort = lib.mkOption {
      type = lib.types.port;
      default = 8081;
      description = "Port for xmrig API";
    };
        ...
        ExecStart = pkgs.writers.writeBash "mining-exporter" ''
          ...
          fetch_xmrig() {
...

            {
              ...
              REJECTED=$(${pkgs.jq}/bin/jq -r '.results.shares_total // .results.shares_good // 0' /tmp/xmrig.json 2>/dev/null || echo "0")
          ...
          update_metrics() {
            ...
            # Fetch all metrics
            fetch_xmrig ${toString cfg.xmrigPort}

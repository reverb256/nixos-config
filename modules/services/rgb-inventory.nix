...
let
  ...
  hostInventory = inventory.hosts.${hostName} or { expected = []; controlDevices = []; };
  ...
  expectedLines = lib.concatMapStringsSep "\n" (device: ''
    rgb_inventory_expected_info{host="${hostName}",backend="${device.backend}",device_kind="${device.kind}",device_hint="${device.hint}"} 1
    rgb_inventory_control_allowed{host="${hostName}",backend="${device.backend}",device_kind="${device.kind}",device_hint="${device.hint}"} ${if device.controlAllowed then "1" else "0"}
...
METRICS
  ...
  controlAllowlist = pkgs.writeText "rgb-control-allowlist-${hostName}.json"

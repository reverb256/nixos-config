    ...
    script = ''
      ...
      if [ "$HERMES_SUBSTATE" = "running" ]; then
        ...
        if [ "$HERMES_SUBSTATE" = "running" ]; then
          ...
          echo "[Hermes Health Check] ⚠ Hermes Agent is activating (SubState: $HERMES_SUBSTATE)"
          exit 1
        fi
      else
        echo "[Hermes Health Check] ✗ Hermes Agent service is not running (ActiveState: $HERMES_ACTIVATED)"
        exit 1
      fi

  ...
  config =
    lib.mkIf cfg.enable {
      environment.etc."health-checks/ai-gateway.sh".text = ''
        #!/bin/sh
        GATEWAY_URL="http://${config.networking.cluster.hosts.zephyr.ip}:${toString config.networking.cluster.kubernetes.nodePorts.ai-gateway}/health"
        TIMEOUT=5
    ...
    // (systemd-helpers.mkTimerService {
      ...
      script = ''
        ...
        if ${pkgs.curl}/bin/curl -f -s --max-time 5 http://${config.networking.cluster.hosts.zephyr.ip}:${toString config.networking.cluster.kubernetes.nodePorts.ai-gateway}/health >/dev/null 2>&1; then

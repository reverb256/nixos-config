  ...
  options.services.ai-inference = {
    ...
    backend = {
      url = mkOption {
        ...
        description = "LM Studio API URL (or vLLM/llama.cpp)";
      };

      type = mkOption {
        type = types.enum [ "lm-studio" "vllm" "llama-cpp" ];
        default = "lm-studio";
    ...
    auth = {
      mode = mkOption {
        type = types.enum [ "none" "tailscale" "api-key" "web3" ];
        default = "none";
      ...
      tailscale = {
        aclTags = mkOption {
          ...
          default = [ ];
          example = [ "tag:ai-inference" "tag:trusted" ];

...
if ZAI_API_KEY_FILE and not ZAI_API_KEY:
    try:
        ...
        print(f"⚠ Failed to read ZAI API key from file: {e}")
...
# Single backend for now (easily extendable to pool)
...
class ModelCache:
    ...
    async def refresh(self, client: BackendClient) -> bool:
        try:
            resp = await client.client.get("/v1/models", timeout=10.0)
            if resp.status_code == 200:
                ...
                async with self._lock:
                    ...
                    models_cache_size.set(len(raw_models))

                # Update router
                router.set_models(raw_models)

                await client.circuit_breaker.record_success()
                print(f"✓ Refreshed model cache: {len(raw_models)} models")
                return True
            else:
...
async def background_refresh():
    """Periodically refresh models and health."""
    while True:
...
async def health():
    ...
    health_data = {
        ...
        "backend": {
            "url": BACKEND_URL,
            "type": BACKEND_TYPE,
            "healthy": backend_client.circuit_breaker.is_healthy(),
            "circuit_state": backend_client.circuit_breaker.state.state.value
        },
        "models": {
            "cached": len(models),
...
async def chat_completions(request: Request):
    """Chat completions with routing, security, failover, and RAG."""
    active_requests.inc()
    ...
    try:
        ...
        if not await backend_client.is_available():
            # Try to refresh and retry once
            await model_cache.refresh(backend_client)
            if not await backend_client.is_available():
                request_counter.labels(model=selected_model, status="unavailable", backend=BACKEND_URL, auth_mode=auth_result["tier"]).inc()
        ...
        if stream:
            async def generate():
                try:
                    async with backend_client.client.stream(
            ...
            backend_resp = await backend_client.request("POST", "/v1/chat/completions", json=body)
            ...
            if backend_resp.status_code == 200:
                ...
                if usage:
                    tokens_generated.labels(model=selected_model, backend=BACKEND_URL).inc(
                        usage.get("completion_tokens", 0)
                    )

            duration = time.time() - start_time
            request_duration.labels(model=selected_model, backend=BACKEND_URL).observe(duration)
            ...
            if rag_used and isinstance(result, dict):
...
'';
  ...
  gatewayPkg = pkgs.runCommand "ai-inference-gateway-pkg" {
  ...
  config = mkIf (cfg.enable && cfg.gateway.enable) {
    systemd.services.ai-inference-gateway = {
      ...
      environment = {
        ...
        LM_STUDIO_API_KEY = if cfg.backend.lmStudio.apiKeyFile != null
          then "" # Will be loaded from file by gateway
          else cfg.backend.lmStudio.apiKey;
        LM_STUDIO_API_KEY_FILE = lib.optionalString (cfg.backend.lmStudio.apiKeyFile != null) cfg.backend.lmStudio.apiKeyFile;
        # ZAI backend configuration
        ZAI_API_KEY = if cfg.backend.zai.apiKeyFile != null
          then "" # Will be loaded from file by gateway
          else cfg.backend.zai.apiKey;
        ZAI_API_KEY_FILE = lib.optionalString (cfg.backend.zai.apiKeyFile != null) cfg.backend.zai.apiKeyFile;
      ...
      serviceConfig = {
        ...
        ProtectHome = true;
        ReadWritePaths = [ "/tmp" "/var/cache/ai-inference" ]

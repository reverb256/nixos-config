...
class MetricsHelper:
    ...
    def inc_rate_limit_denied(
        self,
        limit_type: str,
        amount: int = 1
    ) -> None:
        """Increment rate limit denial counter."""
        self.rate_limit_denied_total.labels(
            limit_type=limit_type
        ).inc(amount)

    # Security Metrics
    def inc_security_blocked(
        self,
        block_reason: str,
        amount: int = 1
    ) -> None:
        """Increment security block counter."""
        self.security_blocked_total.labels(
    ...
    def inc_circuit_breaker_failures(
        self,
        backend_name: str,
        amount: int = 1
    ) -> None:
    ...
    def set_backend_health(
        ...
        self.backend_health.labels(
            backend_name=backend_name
        ).set(1 if healthy else 0)

    # Load Balancer Metrics
    def inc_load_balancer_selections(
        self,
        backend_name: str,
        amount: int = 1
    ) -> None:

...
let
  ...
  # The bump script — written as a file to avoid Nix '' string interpolation
  bumpScript = pkgs.writeShellScriptBin "unified-bump" (
    # Build the script from a non-interpolated Nix string to avoid
    # escaping issues with shell ${} variables.
    # We use builtins.concatStringsSep with a list to avoid interpolation.

      ...
      steam = {
        ...
        extraPackages = with pkgs;
          [
            ...
            gamemode
            mangohud
            # 2026-08-15: gamescope 3.16.22 spawns its internal Xwayland BY NAME.
            # Xwayland is not on the host system PATH (only xwayland-satellite
            # carries it via its own wrapper), so gamescope's spawned Xwayland
            # fails with 'could not connect to wayland server' and gamescope runs
            # headless — no visible window even though the game renders. Put
            # Xwayland in the Steam FHS PATH where gamescope runs.
            xwayland
            libXcursor
            libXi
            libXinerama

...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."nixos-auto-update.sh".source = pkgs.writeScript "nixos-auto-update.sh" ''
      #!/usr/bin/env bash
    ...
    systemd.timers.nixos-auto-update = {
      ...
      timerConfig = {
        OnCalendar = 
          if cfg.interval == "daily" then
            "daily"
          else if cfg.interval == "weekly" then
            "weekly"
          else if cfg.interval == "monthly" then
            "monthly"
          else
            cfg.interval; # Assume it's a custom calendar specification
        Persistent = true;
      };
    };
  };

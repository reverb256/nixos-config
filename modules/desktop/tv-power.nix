# TV power-state daemon.
#
# Watches the HDMI-A-2 (4K HDR TV) and automatically disables the output +
# switches audio away from HDMI when the TV is turned off, re-enabling with
# HDR config when it comes back on. NOT Plasma-specific: runs under niri on
# zephyr (gated on desktop-ness, not plasma6.enable — gating on Plasma would
# silently kill zephyr's TV power management). Avoids pulling kscreen into
# headless sentry's closure.
{ pkgs, lib, config, ... }:

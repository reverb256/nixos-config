{
  ...
  noctaliaConfigFile = pkgs.writeText "noctalia-config.toml" ''
    ...
    # ── v4 → v5 TOML migration (2026-08-19) ─────────────────────────────
    # Noctalia v5 ignores settings.json; it only reads TOML from
    # /etc/noctalia (set by NOCTALIA_CONFIG_HOME=/etc on the unit). The v4
    # settings.json carried 24 top-level sections; only [brightness] + [shell]
    # had been migrated. This block restores the sections whose keys ACTUALLY
    # exist in the v5 schema (noctalia 5.0.0-beta.8, config_schema.cpp). v4 keys
    # with no v5 equivalent (general, ui, session_menu, templates, plugins,
    # systemMonitor, and most sub-fields) are intentionally OMITTED — the daemon
    # silently drops unknown keys, so emitting them is dead config. Every key
    # below was verified with: noctalia config validate /etc/noctalia/config.toml
    # Per-urgency notification durations / media / battery toasts / markdown were
    # removed from the v5 schema; history is kept via history_retention_hours = 0.
    # The Stylix palette activation lives in theme-stylix.toml (below) — do NOT
    # duplicate [theme] here. NOTE: the live state file
    # ~/.local/state/noctalia/settings.toml still carries [theme] source="community"
    # which overrides Stylix post-deploy; strip that [theme] block once after deploy.
    ...
    directory = "/home/j_kro/Pictures/Wallpapers/osaka-jade"

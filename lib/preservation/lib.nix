{lib, ...}: rec {
  ...
  missingIntermediatePaths = paths: let
  ...
  mkIntermediateUserDirectories = defaults: files: prefix: directories: let
    partitions = builtins.partition (d: d.inInitrd) (files ++ directories);
    toPaths = map (
      d:
        if builtins.hasAttr "file" d
        then lib.removePrefix prefix d.file
        else d.directory
    );
  ...
  getNonEmptyUserConfigs = forInitrd: stateConfig: let
    preservesAny = userConfig: lib.any (def: def.inInitrd == forInitrd) (userConfig.files ++ userConfig.directories);
  ...
  onlyIntermediates = forInitrd: builtins.filter (conf: conf.how == "_intermediate" && conf.inInitrd == forInitrd);

  # creates tmpfiles.d rules for the `settings` option of the tmpfiles module from a `preserveAtSubmodule`
  mkTmpfilesRules = forInitrd: preserveAt: stateConfig: let
  ...
  mkMountUnits = forInitrd: preserveAt: stateConfig: let
    allDirectories = getAllDirectories stateConfig;
    allFiles = getAllFiles stateConfig;
    mountedDirectories = onlyBindMounts forInitrd allDirectories;
    mountedFiles = onlyBindMounts forInitrd allFiles;
    ...
    fileMounts =
      map (fileConfig: {
        ...
        after =
          if forInitrd
          then ["systemd-tmpfiles-setup-sysroot.service"]
          else ["systemd-tmpfiles-setup.service"];
        wantedBy =
          if forInitrd
          then ["initrd-preservation.target"]
          else ["preservation.target"];
        before =
          if forInitrd
          then ["initrd-preservation.target"]
          else ["preservation.target"];

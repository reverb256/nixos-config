...
stringData:
  ...
  db-password: # inject via agenix
  api-key: $API_KEY  # inject via agenix
  jwt-secret: # inject via agenix
data:
...
stringData:
  tls.crt: |
    ${TLS_CERT}  # Managed via agenix
  tls.key: |
    ${TLS_KEY}  # Managed via agenix
```
  ...
  .dockerconfigjson: |
    {
      "auths": {
        "myregistry.io": {
          "username": "myuser",
          "password": "${REGISTRY_PASSWORD}",  # Inject via agenix
...
stringData:
  ssh-privatekey: |
    ${SSH_PRIVATE_KEY}  # Managed via agenix
```

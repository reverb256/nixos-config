  ...
  gatewayMain = pkgs.writeText "ai-inference-gateway-v2.py" ''
        ...
        if LM_STUDIO_API_KEY_FILE and not LM_STUDIO_API_KEY:
            try:
                ...
                print(f"⚠ Failed to read LM Studio API key from file: {e}")

        # Load ZAI API key from NixOS configuration
        ZAI_API_KEY = cfg.backend.zai.apiKey
        ...
        # ZAI-specific configuration (for enhanced features)
        ZAI_MAX_RETRIES = int(os.getenv("ZAI_MAX_RETRIES", "3"))
        ZAI_RETRY_DELAY = float(os.getenv("ZAI_RETRY_DELAY", "1.0"))
        ZAI_TIMEOUT = float(os.getenv("ZAI_TIMEOUT", "300.0"))
        ZAI_ENABLE_RETRY = os.getenv("ZAI_ENABLE_RETRY", "true").lower() == "true"
                ...
                async with self._lock:
                    if self.state.state == CircuitState.OPEN:
                        ...
                        if time.time() - self.state.last_failure_time >= self.config.recovery_timeout:
                            ...
                            print(f"⚠ Circuit breaker HALF_OPEN for {self.backend_url}")
                            return True
                        return False
                ...
                async with self._lock:
                    ...
                    if self.state.state == CircuitState.HALF_OPEN:
                        ...
                        if self.state.half_open_calls_made >= self.config.half_open_calls:
                            ...
                            print(f"✓ Circuit breaker CLOSED (recovered) for {self.backend_url}")
                ...
                async with self._lock:
                    ...
                    if self.state.failures >= self.config.failure_threshold:
                        if self.state.state != CircuitState.OPEN:
                            ...
                            print(f"✗ Circuit breaker OPEN for {self.backend_url} ({self.state.failures} failures)")
        ...
        class BackendClient:
            ...
            def __init__(self, url: str, headers: Dict[str, str] = None, is_zai: bool = False):
                self.url = url
                self.headers = headers or {}
                self.is_zai = is_zai
                self.client = httpx.AsyncClient(base_url=url, timeout=ZAI_TIMEOUT if is_zai else 300.0, headers=headers)
            ...
            async def request(self, method: str, path: str, **kwargs) -> httpx.Response:
                ...
                # Use ZAI retry logic if enabled and this is a ZAI client
                if self.is_zai and ZAI_ENABLE_RETRY:
                    return await self._request_with_retry(method, path, **kwargs)
                else:
                    return await self._request_standard(method, path, **kwargs)

            async def _request_standard(self, method: str, path: str, **kwargs) -> httpx.Response:
                """Standard request without retry logic."""
                try:
            ...
            async def _request_with_retry(self, method: str, path: str, **kwargs) -> httpx.Response:
                """ZAI-specific request with exponential backoff retry logic."""
                last_error = None

                for attempt in range(ZAI_MAX_RETRIES + 1):
                    try:
                        if not await self.circuit_breaker.can_request():
                            raise HTTPException(
                                status_code=503,
                                detail=f"ZAI service temporarily unavailable (circuit breaker open)"
                            )
                        ...
                        if response.status_code == 401:
                            ...
                            raise HTTPException(status_code=401, detail=f"ZAI {error_msg}")

                        elif response.status_code == 429:
                            # Rate limit - retry with exponential backoff
                            if attempt < ZAI_MAX_RETRIES:
                                wait_time = ZAI_RETRY_DELAY * (2 ** attempt)
                                print(f"⚠ ZAI rate limit, waiting {wait_time:.1f}s (retry {attempt + 1}/{ZAI_MAX_RETRIES})")
                                await asyncio.sleep(wait_time)
                                continue
                            else:
                                await self.circuit_breaker.record_failure()
                                raise HTTPException(status_code=429, detail="ZAI rate limit exceeded")

                        elif response.status_code >= 500:
                            # Server error - retry with backoff
                            await self.circuit_breaker.record_failure()
                            if attempt < ZAI_MAX_RETRIES:
                                wait_time = ZAI_RETRY_DELAY * (1.5 ** attempt)
                                print(f"⚠ ZAI server error {response.status_code}, waiting {wait_time:.1f}s (retry {attempt + 1}/{ZAI_MAX_RETRIES})")
                                await asyncio.sleep(wait_time)
                                continue
                            else:
                                raise HTTPException(
                                    status_code=response.status_code,
                                    detail=f"ZAI server error after {ZAI_MAX_RETRIES} retries"
                                )
                        ...
                        last_error = e
                        if attempt < ZAI_MAX_RETRIES:
                            wait_time = ZAI_RETRY_DELAY * (1.2 ** attempt)
                            print(f"⚠ ZAI timeout, waiting {wait_time:.1f}s (retry {attempt + 1}/{ZAI_MAX_RETRIES})")
                            await asyncio.sleep(wait_time)
                            continue
                        else:
                            await self.circuit_breaker.record_failure()
                            raise HTTPException(status_code=504, detail=f"ZAI timeout after {ZAI_MAX_RETRIES} retries")

                    except httpx.ConnectError as e:
                        last_error = e
                        if attempt < ZAI_MAX_RETRIES:
                            wait_time = ZAI_RETRY_DELAY * (1.5 ** attempt)
                            print(f"⚠ ZAI connection error, waiting {wait_time:.1f}s (retry {attempt + 1}/{ZAI_MAX_RETRIES})")
                            await asyncio.sleep(wait_time)
                            continue
                        else:
                            await self.circuit_breaker.record_failure()
                            raise HTTPException(status_code=503, detail=f"ZAI connection failed after {ZAI_MAX_RETRIES} retries")

                raise HTTPException(
                    status_code=500,
                    detail=f"ZAI request failed: {str(last_error)}"
                )
                ...
                if await self.primary.is_available():
                    try:
                        ...
                        if e.status_code == 503 and self.fallback:
                            print(f"⚠ Primary unavailable, trying fallback to ZAI...")
                            self.fallback_used = True
                            return await self.fallback.request(method, path, **kwargs)
                        raise
                elif self.fallback:
                    # Primary circuit breaker open, use fallback
                    print(f"⚠ Circuit breaker open, using fallback to ZAI...")
                    self.fallback_used = True
                    return await self.fallback.request(method, path, **kwargs)
                else:
        ...
        backend_client = BackendClient(BACKEND_URL, backend_headers, is_zai=(BACKEND_TYPE == "zai"))
        ...
        if ZAI_API_KEY and ZAI_BASE_URL and ROUTING_ENABLED:
            ...
            fallback_client = BackendClient(ZAI_BASE_URL, fallback_headers, is_zai=True)
            print(f"✓ ZAI fallback configured: {ZAI_BASE_URL} (with retry logic)")
            ...
            async def refresh_multi(self, primary: BackendClient, fallback: BackendClient = None) -> bool:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        print(f"✗ Failed to refresh LM Studio models")
                except Exception as e:
                    await primary.circuit_breaker.record_failure()
                ...
                if fallback:
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            print(f"✗ Failed to refresh ZAI models")
                    except Exception as e:
                        await fallback.circuit_breaker.record_failure()
    ...
    - Match the exact structure specified'''
        ...
        async def startup():
            ...
            print(f"✓ Gateway v2 started on {GATEWAY_HOST}")
            print(f"  Backend: {BACKEND_TYPE} at {BACKEND_URL}")
            print(f"  Fallback: {ZAI_BASE_URL if backend.fallback else 'disabled'}")
            print(f"  Auth: {AUTH_MODE} | Rate limit: {RATE_LIMIT_ENABLED}")
            print(f"  Models: {len(models)} available")
            print(f"  Routing: {'enabled' if ROUTING_ENABLED else 'disabled'}")
            if RAG_ENABLED and rag_engine.enabled:
                print(f"  RAG: enabled (Qdrant: {QDRANT_URL}, Hybrid: {HYBRID_SEARCH_ENABLED})")
            ...
            async def list_tools(self, server_name: str = None) -> List[Dict]:
                ...
                for server in servers_to_query:
                    ...
                    try:
                        ...
                        print(f"✗ MCP server {server.name} error: {e}")
                        if server.name in self.tools_cache:
                            tools.extend(self.tools_cache[server.name])
                return tools
            ...
            async def add_document(
                ...
                try:
                    ...
                    if scoped_collection not in collection_names:
                        ...
                        print(f"  ✓ Created Qdrant collection: {scoped_collection}")
        ...
        class AnthropicMessageTranslator:
            ...
            # Anthropic response IDs use a specific format
            @staticmethod
            def generate_anthropic_id() -> str:
            ...
            def anthropic_to_openai(body: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
                ...
                system_prompt = body.pop("system", None)
                if system_prompt:
                    metadata["has_system"] = True
                    # Prepend system message
                    messages = body.get("messages", [])
                    body["messages"] = [{"role": "system", "content": system_prompt}] + messages

                # Handle extended thinking feature
                ...
                if thinking:
                    ...
                    if isinstance(thinking, dict) and thinking.get("type") == "enabled":
                        ...
                        sys_msg = {"role": "system", "content": f"Think through this step by step. You have a budget of {budget} tokens for reasoning."}
                        body["messages"] = [sys_msg] + body.get("messages", [])

                # Handle Anthropic-specific tool format
                # Anthropic uses same tool format as OpenAI, so no conversion needed
                ...
                if anthropic_model.startswith("claude-"):
                    # Map Claude models to available ZAI and LM Studio models
                    model_mapping = {
                        # High-end Claude models → ZAI's best models
                        "claude-sonnet-4-20250514": "glm-5",  # GLM-5 (200K context) - highest quality
                        "claude-sonnet-4": "glm-5",
                        "claude-opus-4-20250514": "glm-5",

                        # Mid-tier Claude models → ZAI mid-tier or local
                        "claude-sonnet-4-20250514-simplified": "glm-4.7",  # GLM-4.7 (200K context)
                        "claude-sonnet-3-5-20250514": "glm-4.7",

                        # Lightweight Claude models → ZAI fast model or local
                        "claude-haiku-4-20250514": "glm-4.5-air",  # GLM-4.5 Air (128K context) - fast
                        "claude-haiku-4": "glm-4.5-air",

                        # Fallback to local model if ZAI unavailable
                        "claude-3-5-sonnet-20250514": "qwen3.5-35b-a3b@q4_k_m",  # Local 35B model
                        "claude-3-5-sonnet": "qwen3.5-35b-a3b@q4_k_m",
                    }
                    if anthropic_model in model_mapping:
                        body["model"] = model_mapping[anthropic_model]
                        metadata["original_model"] = anthropic_model
                        metadata["mapped_to"] = model_mapping[anthropic_model]

                return body, metadata

            @staticmethod
            def openai_to_anthropic(response: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
                """
                ...
                """
                # Generate Anthropic-style ID
                message_id = metadata.get("anthropic_message_id", AnthropicMessageTranslator.generate_anthropic_id())
                ...
                # Convert content to Anthropic content blocks format
                # Handle both simple text and tool calls
                content_blocks = []

                # Check for tool calls in OpenAI response
                ...
                if tool_calls:
                    for tool_call in tool_calls:
                        ...
                        if not tool_id:
                            ...
                            tool_id = "toolu_" + hashlib.md5(str(tool_call).encode()).hexdigest()[:16]
                ...
                if content:
                    # If there were tool calls, text might be empty or contain reasoning
                    content_blocks.append({
                        "type": "text",
                        "text": content
                    })

                # If no content blocks yet (empty response), add empty text block
                ...
                # Construct Anthropic response
                anthropic_response = {
                    "id": message_id,
                    "type": "message",
                    "role": "assistant",
                    "content": content_blocks,
                    "model": metadata.get("original_model", response.get("model", "claude-sonnet-4-20250514")),
                    "stop_reason": stop_reason,
                    "usage": anthropic_usage
                }

                # Add optional Anthropic-specific fields
                if metadata.get("has_thinking"):
                    anthropic_response["thinking"] = {
                        "type": "enabled",
                        "budget_tokens": 1024
                    }

                # Preserve gateway metadata if present
                if "gateway_routing" in response:
                    anthropic_response["_gateway"] = response["gateway_routing"]
                if "prediction_stats" in response:
                    anthropic_response["_prediction_stats"] = response["prediction_stats"]

                return anthropic_response

            @staticmethod
            def translate_streaming_chunk(chunk: str, metadata: Dict[str, Any]) -> str:
                """
                Translate OpenAI streaming chunk to Anthropic format.

                This is a simplified version - full streaming support would require
                parsing SSE events and converting each one.
                """
                # For now, pass through OpenAI format chunks
                # A full implementation would parse and convert SSE events
                return chunk
        ...
        async def anthropic_messages(request: Request):
            ...
            try:
                ...
                # Log for debugging
                print(f"[Anthropic API] Request: version={anthropic_version}, request_id={request_id}")
                ...
                translator = AnthropicMessageTranslator()
                openai_body, anthropic_metadata = translator.anthropic_to_openai(body.copy())

                # Generate message ID for response
                anthropic_metadata["anthropic_message_id"] = translator.generate_anthropic_id()

                # Process through existing gateway logic
                # Process structured output (if any)
                openai_body, structured_metadata = structured_output_handler.process_request(openai_body)

                # Process tools (if any)
                openai_body, tools_metadata = tools_handler.process_request(openai_body)
                ...
                if RAG_ENABLED and messages:
                    ...
                    if last_user_msg:
                        auth_token = rag_engine._get_token_from_request(request)
                        collection = openai_body.get("rag_collection", "default")

                        use_rag = openai_body.get("use_rag")
                        if use_rag is None:
                            use_rag = rag_engine.should_use_rag(last_user_msg, auth_token)
                        ...
                        if use_rag:
                            retrieval_result = await rag_engine.hybrid_search(
                                query=last_user_msg,
                                collection=collection,
                                top_k=openai_body.get("rag_top_k", RAG_TOP_K),
                                token=auth_token
                            )
                            ...
                            if retrieval_result.chunks:
                                ...
                                system_prompt = openai_body.get("rag_system_prompt",
                                    "Use the following retrieved context to answer the user's question. " +
                                    "If the context doesn't contain relevant information, say so.")
                                ...
                                openai_body["messages"] = [context_message] + messages
                ...
                if stream:
                        ...
                        try:
                            async with effective_backend.client.stream(
                                ...
                                async for chunk_bytes in backend_resp.aiter_bytes():
                                    ...
                                    # For now, pass through OpenAI streaming format
                                    # A full implementation would convert SSE events
                                    yield chunk_bytes
                            ...
                            time_to_first_token.labels(
                                model=selected_model,
                                backend=effective_backend.url
                            ).observe(prediction_stats.time_to_first_token)
                    ...
                    if backend_resp.status_code == 200:
                        ...
                        prediction_stats.completion_tokens = usage.get("completion_tokens", 0)
                        prediction_stats.total_tokens = usage.get("total_tokens", 0)
                        prediction_stats.stop_reason = choices[0].get("finish_reason", "unknown") if openai_response.get("choices") else "unknown"
                    ...
                    # Add enhanced metadata to OpenAI response
                    if isinstance(openai_response, dict):
                        if rag_used:
                            openai_response.setdefault("usage", {})["rag_sources_used"] = len(rag_context.split("[Source")) if rag_context else 0
                            openai_response["rag_metadata"] = {
                                "enabled": True,
                                "retrieval_method": "hybrid" if HYBRID_SEARCH_ENABLED else "vector",
                                "context_length": len(rag_context)
                            }

                        openai_response["gateway_routing"] = {
                            "backend": "zai" if effective_backend == backend.fallback else "lm-studio",
                            "backend_url": effective_backend.url,
                            "model": selected_model,
                            "routing_reason": route_decision.reason if ROUTING_ENABLED else "default"
                        ...
                        if structured_metadata.get("structured_output"):
                            openai_response["structured_output"] = {
                                "enabled": True,
                                "type": structured_metadata.get("type"),
                                "validated": True
                            }
                        ...
                        if tools_metadata.get("has_tools"):
                            openai_response["tools_metadata"] = {
                                "tools_count": tools_metadata.get("tools_count", 0),
                                "tool_choice": tools_metadata.get("tool_choice", "auto")
                            }

                        openai_response["prediction_stats"] = prediction_stats.to_dict()

                    # Translate OpenAI response to Anthropic format
                    anthropic_response = translator.openai_to_anthropic(openai_response, anthropic_metadata)
  ...
  config = mkIf (cfg.enable && cfg.gateway.enable) {
    systemd.services.ai-inference-gateway = {
      ...
      environment = {
        ...
        ZAI_TIMEOUT = builtins.toString cfg.backend.zai.timeout;
        ZAI_ENABLE_RETRY = lib.boolToString cfg.backend.zai.enableRetry;
        PYTHONUNBUFFERED = "1";

...
{...}: {
  ...
  services = {
    # Read-only fleet RGB inventory. Stylix sync enabled for Sapphire GPU control.
    rgb-inventory = {
      enable = true;
    ...
    # PeakMiner 2.8.0 exposes a localhost-only /summary API; cluster
    # Prometheus does not scrape the per-instance API ports.

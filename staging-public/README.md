...
### **Branch Strategy**
- **`main`**: Primary development branch (validated by GitHub Actions)
- **`infra`**: Production deployment branch (auto-synced from main when validated)
- **GitHub Actions**: Validates `main` → auto-merges to `infra` on success
- **Deployment**: Manual execution of `just cluster-deploy` pulls from `main`
- **Feature branches**: Use descriptive names for new features/fixes

### **Privacy & Parameterization**
This repository demonstrates best practices for maintaining public infrastructure configurations while protecting private deployment details. See `PARAMETERIZATION_BEST_PRACTICES.md` for the approach to separating public patterns from private values.

- **Public patterns**: Reusable infrastructure modules and configurations
- **Private values**: Specific IP addresses, hostnames, and deployment details
- **Parameterization**: Using NixOS options system with dummy defaults for public repo
- **Security**: Automated validation prevents private data from entering public configs

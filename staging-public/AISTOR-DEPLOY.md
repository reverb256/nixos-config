...
## Architecture
- **Server**: nexus (192.168.100.X:9000) - AIStor/MinIO server
...
This will:
- Start MinIO/AIStor on 192.168.100.X:9000
...
curl http://192.168.100.X:9000/minio/health/live
...
# Set up alias
mc alias set aistor http://192.168.100.X:9000 minioadmin your-secure-password
...
nix store info --store s3://nix-cache?endpoint=http://192.168.100.X:9000
...
services.openclaw.nginx = {
  ...
  allowedIPs = [ "127.0.0.1" "::1" "192.168.101.X/8" "192.168.0.0/16" ];
};
```
...
3. **IP allowlisting**: Restrict nginx access to known IPs
   ```nix
   services.openclaw.nginx.allowedIPs = [ "192.168.101.X/8" "192.168.0.0/16" ];
   ```

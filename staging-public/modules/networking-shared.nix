# Networking Shared Module - DNS, Firewall, Analytics Blocking, Avahi
# All hosts get this - no hardcoded IPs
# Host-specific networking (IPs, interfaces) goes in hosts/*/default.nix
{lib, ...}:
with lib; {
  # ============================================================================
  ...
  networking.extraHosts = ''
    ...
    # Local network hosts (can be overridden per-host if needed)
    # 192.168.100.X nexus
    # 192.168.100.X forge
    # 192.168.100.X sentry
  '';
    ...
    settings = {
      server = {
        ...
        access-control = [
          "127.0.0.0/8 allow"
          "::1/128 allow"
          "192.168.100.X/24 allow" # Local network access
        ];
        do-tcp = true;
        do-udp = true;
        prefetch = true;

...
- MinIO/AIStor server on 192.168.100.X:9000 (S3 API)
- Console access on 192.168.100.X:9001
...
### Prerequisites
Ensure you have:
- Access to nexus (192.168.100.X) and zephyr (192.168.100.X)
...
# Setup mc alias
mc alias set aistor http://192.168.100.X:9000 minioadmin your-password
...
│  192.168.100.X:9000│  │  sync    │  │   (gdrive, b2)   │
└─────────────────┘  └──────────┘  └──────────────────┘
          │
          │ Internal Network
          ▼
...
- ✅ **AIStor internal**: Server only on internal network (192.168.100.X)
- ✅ **Health monitoring**: 30-second checks with auto-restart
...
nix build .#myPackage
# Subsequent builds use cache from 192.168.100.X:9000
```

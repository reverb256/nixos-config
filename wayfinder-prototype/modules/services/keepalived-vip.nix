...
#   - options + config in ONE inner module (skeleton Q2 → A)
#   - reads config.networking.cluster → `network-constants` is plumbing imported
#     by path in `base` (dissolve Q3); this feature is a CONSUMER and requires the
#     host to also import `base` (host-wiring Q3 → B: explicit dependency imports).
{ inputs, ... }: {
  flake.modules.nixos.keepalived-vip = {
    config,
    lib,
    pkgs,
    ...

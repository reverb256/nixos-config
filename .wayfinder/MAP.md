...
- [execute-zephyr-cutover](tickets/execute-zephyr-cutover.md) — OPEN (drafted, HITL, NOT executed): concrete 6-step handoff for host #1. STEP 0 preflight (read-only) → STEP 1 convert REAL zephyr on the cutover branch using B template → STEP 2 eval-verify (flake check + build + hostName + k3s guard false) → STEP 3 isolated commit (NO `git add -A`) → STEP 4 deploy = **awaits j_kro GO** → STEP 5 Full post-deploy gate (is-system-running + k3s inactive + desktop up) → STEP 6 done-or-rollback. Separate concerns noted: Noctalia VRAM-OOM unmitigated, alloy `;` bug, forge miners revenue-critical (nexus must prove builder first). Reuses this checklist for nexus/forge/sentry.

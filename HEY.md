...
|-------|-------|---------------------|------------|--------|------------|
| j_kro-error-hunt (2026-08-13) | Cluster error hunt + k8s admission/scheduling fixes + intention-comment audit + GH issue/doc sync | kubernetes-manifests/ (nix-csi archive, nvidia-device-plugin, gateway-secrets), modules/services/ (nixos-sync, k8s-secret-sync, dcgm-exporter, bonsai), docs/current-state.md | Peakminer/mining services, MapleSpike app code, home-manager-config, sibling PR branches | ✅ nix-csi removed (#213 closed), device-plugin fixed (GPUs advertised), k8s-secret-sync namespace-ensure, nixos-sync openssh, dcgm RuntimeDirectory, sentry bonsai DSpark; 🔄 4 new issues filed (#463-466); PRs #457-462 are sibling-owned — do not merge | 2026-08-13 (UTC) |
